...
class Sprite:
    ...
    def collide(self, other_object):
        ...
        if dist(self.get_position(), other_object.get_position()) < min_distance:
            return True
        else:
            return False
    ...
    if started:
        if len(rock_group) < 12:
            while True:
                ...
                # Checking if new rock collides with ship
                min_distance = a_rock.radius + my_ship.radius
                if dist(a_rock.get_position(), my_ship.get_position()) > min_distance:
                    break
